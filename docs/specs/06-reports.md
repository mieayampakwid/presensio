...
Status: draft v1.5 (2026-09-21) — v1.5 adds the academic-year dimension to the class report (spec 07); attribution is date-effective via enrollments (spec 02 v2.0).
...
2. **Class report**: pick academic year + class + range (the class list is per selected year, spec 07; attribution is date-effective via enrollments) → per-status counts and rate per student, plus a students × dates grid showing each day's status.
