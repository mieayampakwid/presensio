...
6. **Profile Updates Restricted:** Users can only change their password. They **cannot** modify their `username` or `role` (constrained to Admin). Sole exception: guardians may edit their **own contact fields** on their guardian profile (`phone_number`, `address`, `work`) — the data they are the source of truth for; name and child relations remain admin-managed (see spec 02).
...
- Claim-based guardian self-registration (v1.x candidate): families claim a `parent` account via a per-student invite code + WhatsApp OTP instead of admin provisioning; admin manual provisioning stays as fallback. Not open self-registration — nothing enters the system without the school-issued code (NIS + DOB alone is guessable by classmates and proves nothing about parenthood).