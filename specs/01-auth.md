...
- Built on a modern web application architecture utilizing secure session-based authentication.
...
1. **Login:** Users authenticate with `identification_number` + password; endpoints must be protected by standard rate-limiting against brute force attacks.
...
8. **Data Scoping:** Enforced by application-level authorization rules (policies), not just UI navigation:
