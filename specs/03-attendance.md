...
- Teachers may create/edit records for **their own class for any date**; admins for any class. The system checks the `teachers` profile linked to the logged-in user to determine their homeroom classes based on configurable rules.
...
1. Teacher dashboard shows their class with a date picker (defaults to today) and the marking form: one status control per enrolled student (displayed using `full_name`).
...
3. A student enrolled mid-range only gets records from their enrollment onward; enrollment is effective-date-less (`class_id` on the student record).
